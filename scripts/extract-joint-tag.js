...
argv._.forEach(filename => {
  fs.readFile(filename, (err, data) => {
    parser.parseString(data, (err, result) => {
      ...
      const jointMap = Object.fromEntries(filtered.map((v,i) => [i,v]));
      ...
      const output = argv.sort ? 'urdf.json' : 'urdf.json'; // 'urdfmap.json';
	...
	linkArray.map(link => [link.$.name, link])
      ...
      //
      console.log('mesh files:', Object.entries(linkMap).map(([name,value])=>{return	value?.visual?.map(vis=>vis.geometry?.mesh?.$.filename)}).flat().filter(v=>v));


// (linkName => { linkMap[linkName].visual.forEach(visual => { visual.geometry.mesh.$.filename })
// 								 });
//       console.log('The mesh filenames are:', meshFiles);
   });
  });
});
